...
1. 프로젝트 개요
   ...
   - 대상 사용자
   - 문제 정의
   - 로컬 장난감 데모가 아니라 로그인 사용자별 데이터가 저장되는 SaaS형 웹앱을 목표로 한 이유
...
```txt
최종 목표는 로컬에서만 실행되는 데모가 아니라, Supabase Auth와 Postgres에 사용자별 프로필, 메모리, 채팅, 일정, LLM 활용 기록을 저장하는 SaaS형 학업 내비게이터다. 로컬 fallback은 외부 서비스 장애에 대비한 안전장치로 설명하고, 제출 전에는 가능한 live smoke 결과를 별도로 제시한다.
```
