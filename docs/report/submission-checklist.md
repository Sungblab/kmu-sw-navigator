...
- `pnpm supabase:schema-check`: `profiles`, `user_memories`, `memory_events`, `chat_sessions`, `chat_messages`, `assignments`, `google_oauth_tokens`, `llm_usage_logs`, `document_chunks`, `search_document_chunks_text`, `match_document_chunks` 모두 missing
...
1. `docs/contributing/supabase-live-apply.md` 순서대로 Supabase Dashboard SQL Editor에서 `supabase/schema.sql`을 적용하고 결과를 기록. 현재 schema check는 필수 table/function 전체 missing이므로 최소 `profiles`, `user_memories`, `memory_events`, `chat_sessions`, `chat_messages`, `assignments`, `google_oauth_tokens`, `llm_usage_logs`, `document_chunks`, `search_document_chunks_text`, `match_document_chunks`가 필요하다.
