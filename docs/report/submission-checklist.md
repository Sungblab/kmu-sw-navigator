...
| SaaS 저장 구조 | Supabase Auth/Postgres, `backend/app/services/supabase_stores.py` | 사용자별 profile/memory/chat/assignment/llm log 저장을 live smoke로 검증 예정 |
...
- `pnpm env:check`: Backend Supabase Direct와 Frontend Supabase Framework는 ready, `SUPABASE_JWT_SECRET`, `GEMINI_API_KEY`, Google OAuth 값은 missing
- `pnpm live:smoke-plan`: Supabase strict, DB smoke, login/API smoke, LLM usage smoke, Gemini/Google live smoke가 blocker를 정확히 표시
- `pnpm env:check:strict`: `SUPABASE_JWT_SECRET` 누락으로 exit 2
- Supabase schema readiness probe: backend Direct 값은 있으나 `schema_ready=False`
- `pnpm supabase:smoke`: `--user-id` 또는 `SUPABASE_SMOKE_USER_ID`가 없어 skip exit 2
- `pnpm supabase:login-smoke`: smoke email/password가 없어 skip exit 2. login/API smoke는 추가로 `SUPABASE_JWT_SECRET`과 로컬 FastAPI 서버가 필요
- `pnpm supabase:llm-smoke`: `--user-id` 또는 `SUPABASE_SMOKE_USER_ID`가 없어 skip exit 2
...
1. Supabase 프로젝트에 `supabase/schema.sql` 적용 여부를 확인하고 적용 결과를 기록. 현재 schema probe는 `schema_ready=False`이므로 최소 `profiles`, `user_memories`, `memory_events`, `chat_sessions`, `chat_messages`, `assignments`, `google_oauth_tokens`, `llm_usage_logs`, `document_chunks`, `search_document_chunks_text`, `match_document_chunks`가 필요하다.
2. `backend/.env`에 `SUPABASE_JWT_SECRET`을 추가해 Auth token 검증 경로 완성.
3. Supabase Auth 테스트 유저의 UUID/email/password를 준비하되 repo에 커밋하지 않고 `SUPABASE_SMOKE_USER_ID`, `SUPABASE_SMOKE_EMAIL`, `SUPABASE_SMOKE_PASSWORD` 또는 CLI 인자로만 사용.
4. `frontend/.env`의 Supabase Framework 값으로 실제 가입/로그인 세션 생성 확인.
...
11. 발표영상 녹화 전 `docs/product/demo-scenario.md` 순서대로 리허설.
