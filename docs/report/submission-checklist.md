...
- `pnpm env:check`: Backend Supabase Direct, Backend Supabase Auth Verification, Frontend Supabase Framework, Gemini는 ready. Google OAuth 값은 missing이지만 제출 필수 blocker로 보지 않음
- `pnpm live:smoke-plan`: Supabase env strict, schema check command, smoke user create, DB/login/API/LLM smoke, Gemini smoke는 ready. Google Calendar smoke는 선택 확장 항목으로 분리
...
- `pnpm live:readiness -- --include-seed --api-base http://127.0.0.1:8001`: env, SQL bundle, Supabase schema, API contract, API health 모두 ready. Google OAuth env는 선택 확장 기능용으로만 missing
...
Gemini grounding은 UI/API 경로에서 live smoke를 별도로 기록한다. Google Calendar OAuth는 선택 확장 기능이라 이번 제출 필수 검증에서 제외한다.
