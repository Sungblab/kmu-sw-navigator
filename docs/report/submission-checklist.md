...
1. Supabase 프로젝트에 `supabase/schema.sql` 적용 여부를 확인하고 적용 결과를 기록. 현재 schema check는 필수 table/function 전체 missing이므로 최소 `profiles`, `user_memories`, `memory_events`, `chat_sessions`, `chat_messages`, `assignments`, `google_oauth_tokens`, `llm_usage_logs`, `document_chunks`, `search_document_chunks_text`, `match_document_chunks`가 필요하다.
