...
- `pnpm live:smoke-plan`: Supabase env strict와 Gemini smoke는 ready, Supabase DB/login/API/LLM smoke는 user id/email/password 입력 blocker, Google Calendar smoke는 OAuth/token blocker를 표시
...
- `pnpm supabase:login-smoke`: smoke email/password가 없어 skip exit 2. login/API smoke는 실제 Supabase Auth 계정과 로컬 FastAPI 서버가 필요
...
2. Supabase Auth 테스트 유저의 UUID/email/password를 준비하되 repo에 커밋하지 않고 `SUPABASE_SMOKE_USER_ID`, `SUPABASE_SMOKE_EMAIL`, `SUPABASE_SMOKE_PASSWORD` 또는 CLI 인자로만 사용.
