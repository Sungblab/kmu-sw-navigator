...
신입생 사용 사례를 먼저 보여주지만 구조는 전체 학년까지 확장할 수 있게 설계했습니다.
```
...
- 설정 화면: Supabase 로그인, live readiness 상태. Google Calendar는 선택 확장 기능으로만 언급
...
- 저장 후 일정 목록과 D-day
- 완료/삭제 버튼
- Google Calendar 내보내기 버튼은 누르지 않고 선택 확장 기능으로만 설명
...
- Google Calendar export는 event payload와 OAuth 기반 연결 코드만 준비되어 있으며, 이번 제출 시연에서는 내부 일정 관리 기능만 보여준다.
- OAuth token이 없으면 export 성공으로 처리하지 않고 Google Calendar 연결이 필요하다는 오류를 반환한다.
...
```txt
LLM은 주제 구체화, 설계 문서, 구현 보조, 오류 분석, 데이터 정형화, 코드 리뷰 관점 정리에 사용했습니다.
AGENTS.md, 문서 라우터, Superpowers spec/plan, 전용 Codex 스킬로 개발 하네스를 만들었고,
제출 코드는 그대로 복사한 것이 아니라 Python 로직을 직접 설명할 수 있게 함수, 조건문, 테스트, 주석, 문서로 정리했습니다.
```

## 7. 과제 조건 매핑

| 과제 조건 | 라이브 시연에서 보여줄 위치 |
| --- | --- |
...
| 의미 있는 출력 | 답변, 추천 결과, D-day, 완료/삭제 상태, LLM 활용 기록 |
...
- Google Calendar는 OAuth 기반 단방향 event insert 확장으로 남긴다. 이번 제출에서는 앱 내부 일정 관리가 핵심 기능이다.
- 라즈베리파이 홈서버 배포는 Docker Compose로 앱 서버와 정적 프론트/reverse proxy를 묶는 후속 운영 과제로 남긴다.
- Supabase/Gemini 같은 외부 서비스는 키와 네트워크 상태에 영향을 받으므로 local/live 검증 명령과 녹화 백업을 함께 유지한다.
