...
API는 사용자가 보낸 profile id를 그대로 믿지 않는다. 배포 환경에서는 Supabase access token을 `Authorization` header로 받고, JWT 서명과 audience를 검증한 뒤 payload의 `sub`만 user id로 사용한다.

| 환경 | 입력 | user id 결정 |
| --- | --- | --- |
| `SUPABASE_JWT_SECRET` 있음 | `Authorization: Bearer <token>` | 검증된 JWT의 `sub` |
| `SUPABASE_JWT_SECRET` 없음 | `X-User-Id` | 로컬 테스트와 발표용 dev fallback |

이 분기를 둔 이유는 실제 서비스에서는 사용자가 임의로 user id를 바꾸지 못하게 하면서도, 과제 테스트와 오프라인 발표에서는 외부 Supabase 로그인 없이 Python API를 실행할 수 있게 하기 위해서다.
...
| 실행 환경 | 저장소 | 이유 |
| --- | --- | --- |
| Supabase 환경 변수와 schema readiness probe 통과 | `SupabaseProfileStore`, `SupabaseMemoryStore` | 실제 서비스와 데모에서 프로필/메모리를 영속 저장 |
| Supabase 환경 변수 없음 또는 schema 미적용 | `InMemoryProfileStore`, `InMemoryMemoryStore` | 외부 키 누락/DB 미준비/발표 장애 시에도 Python API 동작을 검증할 수 있는 보조 경로 |
...
- token이 없으면 synthetic id로 앱 내부 export 상태만 저장해 로컬 데모를 유지한다.
...
현재 구현은 외부 키가 없어도 in-memory fallback과 deterministic 응답으로 실행 가능하고, 키가 있으면 Supabase/Gemini/Google adapter가 live 경로를 사용하도록 설계되어 있다. 제출 목표는 fallback 가능성을 완성으로 주장하는 것이 아니라, live 연결이 가능한 항목을 실제 smoke로 확인하고 결과를 문서에 남기는 것이다.
