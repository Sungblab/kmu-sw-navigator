...
| 조건문 | `classify_memory_sensitivity()`, `extract_learning_context()`, `detect_intent()` | 입력 문장에 포함된 키워드에 따라 메모리 저장 여부, 학습/진로 정보 추출, 채팅 상담 유형을 분기한다. |
| 반복문 | `any(...)`, list comprehension, active memory filtering | 키워드 목록과 메모리 목록을 순회해 조건에 맞는 항목을 찾는다. |
| 함수 | `create_memory_candidate()`, `create_learning_context_memory_from_chat()`, `build_chat_response()` | 기능을 설명 가능한 단위로 분리해 테스트와 발표 설명이 가능하게 했다. |
...
## 2-1. 대화 기반 학습 컨텍스트 추출

구현 위치:

- `backend/app/services/memory_service.py`
- `extract_learning_context()`
- `create_learning_context_memory_from_chat()`

AI 상담에서 사용자가 관심 분야, 목표, 코딩 경험, 선호 활동을 말하면 추천 입력에 필요한 낮은 민감도 정보만 Python 규칙으로 추출한다. 예를 들어 “AI랑 백엔드에 관심 있고 목표는 포트폴리오 준비야. 코딩은 초급이고 프로젝트로 배우고 싶어. 주 4시간 가능해”라는 문장은 추천용 딕셔너리로 정리된다.

| 필드 | 추출 기준 |
| --- | --- |
| `track_interests` | AI, 백엔드, 프론트엔드, 데이터, 창업 키워드 |
| `activity_interests` | 개발, 알고리즘, 동아리 같은 활동 키워드 |
| `goal` | `목표는 ...` 형태의 짧은 목표 문장 |
| `coding_level` | 초급/중급/고급 관련 표현 |
| `preference` | 프로젝트, 강의, 스터디 선호 표현 |
| `activity_style` | 개인/팀/혼합 활동 방식 표현 |
| `weekly_hours` | `주 N시간` 패턴 |

이 로직은 사용자가 별도 추천 폼을 먼저 채우지 않아도 대화에서 나온 정보를 `conversation / learning_context` 메모리로 저장하고, `ChatResponse.memory_updates`로 프론트엔드에 즉시 알려준다. 비밀번호, 학점, 불안 같은 민감도 판단은 기존 `classify_memory_sensitivity()`를 그대로 통과하므로 저장 정책은 동일하게 유지된다.
