...
| backend-api-foundation | active | `backend/app/` | FastAPI 앱, `/api` 라우터, Supabase JWT/dev fallback 인증 dependency, 저장소 protocol | API 계약 변경은 schema/test/docs를 함께 갱신 |
| supabase-schema | active | `supabase/schema.sql`, `backend/app/services/supabase_stores.py`, `backend/app/scripts/check_env.py` | profiles, document_chunks, assignments, chat/memory/log/calendar token schema, profile/memory persistence adapter, env check | schema 변경은 SQL 파일과 docs를 함께 갱신하고 live DB smoke를 별도 기록 |
...
| navigator-home-ui | complete | `frontend/src/` | Claude/NotebookLM식 3-column workspace shell, sidebar navigation, mobile navigation/context drawers, chat workspace, context panel 1차 구현 | landing/dashboard 대신 상담 workspace를 첫 화면으로 유지 |
| frontend-auth-session | complete | `frontend/src/lib/supabase.ts`, `frontend/src/lib/api.ts`, `frontend/src/App.tsx`, `backend/app/scripts/auth_api_smoke.py`, `backend/app/scripts/supabase_login_smoke.py` | Supabase browser client, API Bearer token forwarding, 설정 화면 로그인/가입/로그아웃 UI, access token/API/login smoke | env가 없으면 demo fallback 유지, live auth smoke는 키와 실제 계정 설정 후 실행 |
...
| llm-usage-log | active | `docs/llm/`, `backend/app/api/llm_logs.py`, `backend/app/services/llm_usage_log_service.py`, `backend/app/scripts/llm_usage_smoke.py`, `frontend/src/` | 개발 과정 기록, 앱 내부 LLM 사용 로그 API, LLM 기록 화면, Supabase insert/list smoke | Codex 사용과 Gemini API 사용을 구분하고 자동 기록 범위를 문서화 |
