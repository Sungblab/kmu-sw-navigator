...
| rag-ingest | active | `backend/app/services/document_ingest.py`, `backend/app/services/embedding_service.py`, `backend/app/scripts/ingest_documents.py`, `backend/app/scripts/prepare_raw_document.py`, `backend/app/scripts/gemini_smoke.py`, `data/`, `docs/architecture/rag-course-data-collection.md` | 사용자 제공 자료 inbox, raw Markdown 정형화, 공식 과목 데이터/비공식 수강신청 팁 분리 수집 기준, raw/wiki chunk payload 생성, Gemini embedding adapter, dry-run, Supabase insert script, Gemini smoke | live embedding ingest는 `GEMINI_API_KEY`와 Supabase env 설정 후 실행 |
| rag-quality-eval | complete | `backend/app/scripts/rag_quality_eval.py`, `backend/tests/test_rag_quality_eval_script.py`, `docs/testing/rag-evaluation.md` | 제출 시연 질문이 local raw/wiki 근거를 잡는지, 기대 category와 핵심어를 만족하는지, 범위 밖 질문을 내부 근거로 과장하지 않는지 확인하는 `pnpm rag:evaluate` | 답변 생성 품질 평가는 아니므로 Gemini/live smoke와 분리하고, 시연 질문이나 RAG 자료가 바뀌면 평가 case를 함께 갱신 |
| rag-chat | active | `backend/app/api/chat.py`, `backend/app/schemas/chat.py`, `backend/app/services/chat_contract_service.py`, `backend/app/services/retrieval_service.py`, `backend/app/services/answer_generation_service.py`, `backend/app/services/chat_store.py`, `backend/app/scripts/gemini_answer_smoke.py`, `frontend/src/` | 질문 답변 contract, local fallback retrieval, Supabase text/vector RPC retrieval evidence, Gemini answer generation/streaming, Google grounding, 상담 모드/응답 품질 선택, 텍스트 첨부 컨텍스트, 답변 중지, chat session/message 저장, recent chats 조회/복원/삭제, 자동 상담 제목, SSE stream 응답, 중복 제거된 번호형 출처 표시 | live Gemini/Supabase/grounding smoke는 키 설정 후 실행 |
| frontend-static-screens | retired | `docs/superpowers/specs/`, `docs/superpowers/plans/` | 로그인/메인 화면의 초기 정적 설계 기록 | 실제 UI는 개인화 SW 내비게이터 React 구현으로 대체 |
| chatbot-evidence-visualization | retired | `docs/superpowers/specs/`, `docs/superpowers/plans/` | 챗봇 답변 옆 출처, 로드맵, 추천 이유 시각화 기록 | 근거 표시 UX는 `personalized-sw-navigator-design`에 통합 |
...
| user-profile-memory | complete | `backend/app/api/`, `backend/app/services/`, `backend/app/schemas/`, `supabase/schema.sql`, `frontend/src/` | 필수 프로필, 관심사 온보딩, 메모리 민감도 정책, memory_events, 메모리 생성/조회/수정/삭제, chat contract 1차 구현, Supabase persistence adapter | 환경 변수가 없으면 in-memory fallback을 쓰며 live Supabase smoke는 별도 검증 필요 |
...
| frontend-auth-session | complete | `frontend/src/lib/supabase.ts`, `frontend/src/lib/api.ts`, `frontend/src/App.tsx`, `backend/app/scripts/auth_api_smoke.py`, `backend/app/scripts/supabase_login_smoke.py`, `backend/app/scripts/create_smoke_user.py` | Supabase browser client, 로그인/가입/로그아웃/온보딩 gate, API Bearer token forwarding, smoke Auth user 생성, access token/API/login smoke | 로그인 세션 없이는 앱과 API를 사용하지 않으며, live auth smoke는 schema 적용 후 재실행 |
...
| report-presentation | planned | `docs/report/` | 보고서, 발표자료, 라이브 시연 시나리오 | 실행 결과 캡처와 검증 명령을 함께 기록 |
