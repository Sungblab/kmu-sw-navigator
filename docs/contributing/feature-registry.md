...
| backend-api-foundation | active | `backend/app/` | FastAPI 앱, `/api` 라우터, Supabase JWT/dev fallback 인증 dependency, Supabase schema readiness probe, 저장소 protocol | API 계약 변경은 schema/test/docs를 함께 갱신 |
| supabase-schema | active | `supabase/schema.sql`, `backend/app/services/supabase_stores.py`, `backend/app/scripts/check_env.py` | profiles, document_chunks, assignments, chat/memory/log/calendar token schema, profile/memory persistence adapter, env check, schema 미적용 local demo fallback | schema 변경은 SQL 파일과 docs를 함께 갱신하고 live DB smoke를 별도 기록 |
