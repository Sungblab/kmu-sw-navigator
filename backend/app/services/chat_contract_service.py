...
NOTICE_KEYWORDS = ["최신", "공지", "공지사항", "모집", "장학", "행사", "설명회", "특강"]
KMU_NOTICE_CONTEXT_KEYWORDS = [
    "소융대",
    "소프트웨어융합대학",
    "소프트웨어학부",
    "인공지능학부",
...
def detect_intent(message: str, *, mode: str = "auto") -> str:
    ...
    if _contains_any(normalized, SCHEDULE_KEYWORDS):
        return "schedule_assistant"
    # 최신 공지/모집 정보는 static RAG보다 웹 근거가 중요하므로 grounding 대상으로 분리한다.
    if _contains_any(normalized, NOTICE_KEYWORDS) and _contains_any(
        normalized, KMU_NOTICE_CONTEXT_KEYWORDS
    ):
        return "latest_notice_advisor"
