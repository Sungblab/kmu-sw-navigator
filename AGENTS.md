...
1. 응답을 끝내기 전에 문서 갱신 필요 여부를 판단합니다.
   - 기능 동작, API 계약, 검증 결과, 과제 증거, 실행 방법, 제출 설명이 바뀌었으면 관련 문서를 갱신합니다.
   - 코드 주석만 보강했거나, 내부 리팩토링이 사용자/문서 기준을 바꾸지 않으면 문서 갱신을 생략할 수 있습니다.
   - 문서 갱신을 생략하는 경우 최종 답변에 “문서 갱신 불필요”라고 장황하게 쓰지는 말고, 필요 시 검증 결과만 짧게 남깁니다.
2. 관련 문서를 갱신해야 한다고 판단하면 아래 후보 중 필요한 파일만 수정합니다.
...
3. `superpowers:verification-before-completion`을 호출하고 검증 명령을 실행합니다.
4. `superpowers:requesting-code-review`를 호출해 코드 리뷰 관점으로 변경을 점검합니다.
5. PR 기반 작업이면 Gemini Code Assist와 팀원 리뷰를 확인하고, 반영/미반영 이유를 남깁니다.
6. 직접 push 작업이면 최소한 자체 코드 리뷰 결과와 검증 명령을 plan 또는 LLM 활용 기록에 남깁니다.

## 기여 규칙

- 새 작업은 브랜치를 만들어 진행합니다.
- 브랜치 이름 예시: `feature/rag-chat`, `feature/schedule-dday`, `docs/final-report`
- 커밋 메시지 예시:
  - `docs: 초기 문서 구조 추가`
...
- 핵심 로직은 팀원이 읽고 설명할 수 있도록 구조와 책임을 명확히 정리합니다.
...
- 모든 답변을 끝내기 전에는 문서 갱신 필요 여부를 한 번 판단합니다. 필요할 때만 갱신하고, 불필요한 문서 churn은 만들지 않습니다.
...
- ChatGPT, Claude, Gemini, Codex 같은 LLM 사용은 허용되며, 사용 목적과 확인 내용을 기록합니다.
